"""Stage ANI representatives and connect cluster SNP trees to the CLI."""
...
from sgtree.id_schema import assign_genome_ids
from sgtree.input_stage import _list_files, detect_input_format
...
def _list_nucleotide_files(input_dir: str) -> list[Path]:
    ...
    if not directory.is_dir():
        raise ValueError(
            f"ANI clustering currently requires a genome directory, got: {input_dir}"
        )
    return [Path(path).absolute() for path in _list_files(input_dir)]


def _discover_inputs(input_dir: str, source: str) -> list[dict[str, object]]:
    if detect_input_format(input_dir) != "fna":
        raise ValueError(
            f"ANI clustering requires nucleotide genomes for {source}, "
            f"but {input_dir} did not resolve to FNA input"
        )
    rows: list[dict[str, object]] = []
    genome_files = assign_genome_ids(
        [str(path) for path in _list_nucleotide_files(input_dir)]
    )
    for path, genome_id in genome_files:
        contigs, total_bases = _fasta_stats(Path(path))
        rows.append(
            {
                "genome_id": genome_id,
                "input_format": "fna",
...
def prepare_ani_cluster_inputs(cfg: Config) -> pd.DataFrame:
    """Cluster assemblies and stage the selected species-tree representatives."""
    Path(cfg.ani_dir).mkdir(parents=True, exist_ok=True)
    ...
    ani_core.run_ani_clustering(
        ...
        outdir=cfg.ani_dir,
        ani_threshold=cfg.ani_threshold / 100.0
        if cfg.ani_threshold > 1.0
        else cfg.ani_threshold,
    ...
    source_by_genome = {
        str(row["genome_id"]): str(row["source_role"]) for row in query_rows + ref_rows
    }
    path_by_genome = {
    ...
    members["representative_source"] = (
        members["representative_genome"].astype(str).map(source_by_genome)
    )
    members["is_representative"] = (
        members["kept_for_species_tree"].astype(str).str.lower() == "yes"
    )
    ...
    for row in kept.itertuples(index=False):
        target_dir = (
            cfg.ani_selected_ref_dir
            if str(row.source) == "ref"
            else cfg.ani_selected_query_dir
        )
    ...
    cfg.ref_concat = str(Path(cfg.outdir) / "reference_cache")

    print(
        f"-... ANI clustering retained {len(kept)} representative genomes "
        f"from {len(members)} total inputs "
...
def build_cluster_snp_trees(cfg: Config) -> pd.DataFrame:
    """Build marker-guided SNP trees for clusters that meet the size threshold."""
    ...
    summary_path = ani_core.build_snp_trees(
        ...
        cpus=cfg.num_cpus,
        models_path=cfg.models_path if Path(cfg.models_path).exists() else cfg.modeldir,
        hmmsearch_cutoff=cfg.hmmsearch_cutoff,
        hmmsearch_evalue=cfg.hmmsearch_evalue,
        min_contig_ani=max(
            0.95,
            cfg.ani_threshold / 100.0 if cfg.ani_threshold > 1.0 else cfg.ani_threshold,
        ),
    )
