...
The current codebase has been validated in two ways. First, the repository unit suite completed successfully with `15/15` tests passing via `pixi run test-unit`. Second, the cross-engine regression harness in [`tests/regression_parity.py`](/home/fschulz/dev/sgtree/tests/regression_parity.py) completed successfully via `pixi run test-regression-clean`, confirming topology parity, marker-count-matrix parity, and RF-status parity between the Python and Nextflow implementations across the default, reference-assisted, singleton-enabled, and iterative duplicate-cleanup branches.
