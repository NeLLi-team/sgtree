...
`EXTRACT_SEQUENCES` retrieves the selected protein sequences for each marker. Alignment proceeds according to `--aln`. The default path uses `hmmalign`, which keeps each marker aligned against its profile HMM. `mafft` remains available as a de novo alternative; for marker sets with fewer than 100 taxa, each MAFFT alignment runs single-threaded and the workflow parallelizes across markers, whereas larger marker sets use four threads per MAFFT job. `ELIMINATE_DUPLICATES` removes surplus copies based on the score table from parsing. `TRIMAL` applies `-gt 0.1` to remove high-gap columns. `BUILD_SUPERMATRIX` concatenates trimmed marker alignments, and `FASTTREE` infers `tree.nwk`. `WRITE_HEATMAP` produces the iTOL-compatible marker-count annotation file.
